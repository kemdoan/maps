...
function initMap() {
  ...
  if (!('Notification' in window)) {
    ...
    document.getElementById('notif-bar').querySelector('span').textContent =
      '🔕 Thông báo bị chặn — vào cài đặt để bật';
...
async function searchRoute() {
  ...
  const q = destInput.value.trim();
  if (!q) { showToast(' ⚠️ Chưa chọn điểm đến'); return; }
...
async function calcRoute(dest) {
  if (!currentPosition) { showToast('Vui lòng bấm vào 📍 để lấy vị trí'); return; }
...
function notifyCurrentStep() {
  ...
  sendNotification('KD Maps  ' + icon, step.instruction + '\nĐi ' + formatDist(step.distance));
...
function goToMyLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      ...
      updateUserMarker(currentPosition);
      showToast('📍 Đã thấy vị trí');
    },
    err => {
      console.error('GPS:', err);
      showToast('⚠️ Chưa bật vị trí');
    },
...
async function testWatchNotif() {
  ...
  await sendNotification('KD Maps', 'Đang hoạt động');
}
